...
local State = {
    PlayerESP=false, PlayerBoxes=true, PlayerNames=true,
    PlayerDistance=true, PlayerTracers=true,
    PlayerColor=Color3.fromRGB(0,170,255),
    NextbotESP=false, NextbotBoxes=true, NextbotNames=true,
    NextbotDistance=true, NextbotTracers=true,
    NextbotColor=Color3.fromRGB(255,50,50),
    FOVEnabled=false, FOVValue=70, OriginalFOV=70, FOVTween=nil,
    WalkspeedEnabled=false, WalkspeedValue=16, OriginalSpeed=16,
    JumpEnabled=false, JumpValue=50, OriginalJump=50,
    RadarEnabled=false,
...
local function CalcBox(character)
    ...
    return {topV=topV,botV=botV,h=h,w=w,cx=cx,hrpPos=pos,
...
local function InitRadar()
    local vp=Camera.ViewportSize
    local size=200 local pad=10
    local rx=vp.X-size-pad local ry=pad
...
local function UpdateRadar()
    ...
    local size=200 local pad=10
    local rx=vp.X-size-pad local ry=pad
    local cx=rx+size/2 local cy=ry+size/2
    ...
    DS(RadarDrawings.crossH,"From",Vector2.new(rx,cy)) DS(RadarDrawings.crossH,"To",Vector2.new(rx+size,cy))
    DS(RadarDrawings.crossV,"From",Vector2.new(cx,ry)) DS(RadarDrawings.crossV,"To",Vector2.new(cx,ry+size))
...
local function UpdateCrosshair()
    ...
    local s=State.CrosshairSize local gap=4
    local col=State.CrosshairColor local th=State.CrosshairThick
    local function setLine(line,outline,fx,fy,tx,ty)
        if outline then DS(outline,"Visible",vis) DS(outline,"From",Vector2.new(fx,fy)) DS(outline,"To",Vector2.new(tx,ty)) DS(outline,"Thickness",th+1.5) end
...
-- FULLBRIGHT
local OrigBright,OrigAmb,OrigOut,OrigFog=1,Color3.new(0,0,0),Color3.new(0,0,0),100000
local function EnableFullbright()
    OrigBright=Lighting.Brightness OrigAmb=Lighting.Ambient OrigOut=Lighting.OutdoorAmbient OrigFog=Lighting.FogEnd
    Lighting.Brightness=10 Lighting.Ambient=Color3.new(1,1,1) Lighting.OutdoorAmbient=Color3.new(1,1,1)
...
local function EnableNoFog() OrigFog=Lighting.FogEnd Lighting.FogEnd=1e9 Lighting.FogStart=1e9 end
local function DisableNoFog()
    if not State.FullbrightEnabled then Lighting.FogEnd=OrigFog Lighting.FogStart=0 end
end

-- MOVEMENT
local MoveConn=nil
local function ApplySpeed(s) local c=LocalPlayer.Character if not c then return end local h=c:FindFirstChildOfClass("Humanoid") if h then h.WalkSpeed=s end end
local function ApplyJump(j) local c=LocalPlayer.Character if not c then return end local h=c:FindFirstChildOfClass("Humanoid") if h then h.JumpPower=j end end
local function EnableMovement()
    local c=LocalPlayer.Character
    if c then local h=c:FindFirstChildOfClass("Humanoid") if h then State.OriginalSpeed=h.WalkSpeed State.OriginalJump=h.JumpPower end end
    if MoveConn then MoveConn:Disconnect() end
    MoveConn=RunService.Heartbeat:Connect(function()
        local ch=LocalPlayer.Character if not ch then return end
        local hm=ch:FindFirstChildOfClass("Humanoid") if not hm then return end
        if State.WalkspeedEnabled and math.abs(hm.WalkSpeed-State.WalkspeedValue)>0.5 then hm.WalkSpeed=State.WalkspeedValue end
        if State.JumpEnabled and math.abs(hm.JumpPower-State.JumpValue)>0.5 then hm.JumpPower=State.JumpValue end
    end)
    table.insert(Connections,MoveConn)
    Track(LocalPlayer.CharacterAdded:Connect(function(char)
        task.wait(0.1)
        local hm=char:FindFirstChildOfClass("Humanoid") if not hm then return end
        if State.WalkspeedEnabled then hm.WalkSpeed=State.WalkspeedValue end
        if State.JumpEnabled then hm.JumpPower=State.JumpValue end
    end))
end
...
local function TweenFOV(target)
    ...
    local tw=TweenService:Create(Camera,TweenInfo.new(0.22,Enum.EasingStyle.Quad,Enum.EasingDirection.Out),{FieldOfView=target})
...
local function InitNextbots()
    ...
    Track(Workspace.ChildAdded:Connect(function(c)
        task.defer(function()
            ...
            elseif c:IsA("Folder") or c:IsA("Model") then task.wait(0.3) for m in pairs(ScanNextbots()) do AddN(m) end end
        end)
    end))
    ...
    task.spawn(function()
        while task.wait(5) do
            RefreshCharSet()
            for m in pairs(ScanNextbots()) do AddN(m) end
            for m in pairs(NB) do if not m or not m.Parent then DropN(m) end end
        end
    end)
end
    ...
    Track(RunService.RenderStepped:Connect(function()
        for plr,bundle in pairs(PB) do
            if State.PlayerESP and plr~=LocalPlayer and plr.Character and plr.Character.Parent then
                RenderBundle(bundle,plr.Character,plr.DisplayName,{boxes=State.PlayerBoxes,names=State.PlayerNames,distance=State.PlayerDistance,tracers=State.PlayerTracers},State.PlayerColor)
            else BHide(bundle) end
        end
        for model,bundle in pairs(NB) do
            if State.NextbotESP and model and model.Parent then
                RenderBundle(bundle,model,model.Name,{boxes=State.NextbotBoxes,names=State.NextbotNames,distance=State.NextbotDistance,tracers=State.NextbotTracers},State.NextbotColor)
            else BHide(bundle) if not model or not model.Parent then task.defer(function() DropN(model) end) end end
        end
        UpdateRadar()
...
-- GUI
local Win=Rayfield:CreateWindow({Name="Evade | Visuals",LoadingTitle="Evade | Visuals",LoadingSubtitle="by axiom",Theme="Darker",DisableRayfieldPrompts=true,DisableBuildWarnings=true,ConfigurationSaving={Enabled=false},KeySystem=false})

local PT=Win:CreateTab("Players","user")
PT:CreateSection("Player ESP")
PT:CreateToggle({Name="Enable Player ESP",CurrentValue=false,Flag="PE",Callback=function(v) State.PlayerESP=v; if not v then for _,b in pairs(PB) do BHide(b) end end end})
...
PT:CreateColorPicker({Name="Color",Color=Color3.fromRGB(0,170,255),Flag="PCOL",Callback=function(v) State.PlayerColor=v; for _,b in pairs(PB) do BRecolor(b,v) end end})

local NT=Win:CreateTab("Nextbots","zap")
NT:CreateSection("Nextbot ESP")
NT:CreateToggle({Name="Enable Nextbot ESP",CurrentValue=false,Flag="NE",Callback=function(v) State.NextbotESP=v; if not v then for _,b in pairs(NB) do BHide(b) end end end})
...
NT:CreateColorPicker({Name="Color",Color=Color3.fromRGB(255,50,50),Flag="NCOL",Callback=function(v) State.NextbotColor=v; for _,b in pairs(NB) do BRecolor(b,v) end end})
NT:CreateButton({Name="Force Rescan",Callback=function()
    local n=0; for m in pairs(ScanNextbots()) do AddN(m); n=n+1 end
    Rayfield:Notify({Title="Rescan",Content=n.." nextbot(s)",Duration=3})
end})

local VT=Win:CreateTab("Visuals","eye")
VT:CreateSection("Radar")
VT:CreateToggle({Name="Enable Radar",CurrentValue=false,Flag="RAD",Callback=function(v) State.RadarEnabled=v; if not v then SetRadarVisible(false) for _,d in pairs(RadarDrawings.dots) do DS(d,"Visible",false) end end end})
VT:CreateSection("Crosshair")
VT:CreateToggle({Name="Enable Crosshair",CurrentValue=false,Flag="CHR",Callback=function(v) State.CrosshairEnabled=v; if not v then for _,d in pairs(CrosshairDrawings) do DS(d,"Visible",false) end end end})
VT:CreateSlider({Name="Size",Range={4,30},Increment=1,CurrentValue=10,Flag="CHS",Callback=function(v) State.CrosshairSize=v end})
VT:CreateSlider({Name="Thickness",Range={1,4},Increment=1,CurrentValue=1,Flag="CHT",Callback=function(v) State.CrosshairThick=v end})
VT:CreateColorPicker({Name="Crosshair Color",Color=Color3.new(1,1,1),Flag="CHCOL",Callback=function(v) State.CrosshairColor=v end})
VT:CreateSection("Environment")
VT:CreateToggle({Name="Fullbright",CurrentValue=false,Flag="FB",Callback=function(v) State.FullbrightEnabled=v; if v then EnableFullbright() else DisableFullbright() end end})
VT:CreateToggle({Name="No Fog",CurrentValue=false,Flag="NF",Callback=function(v) State.NoFogEnabled=v; if v then EnableNoFog() else DisableNoFog() end end})

local MT=Win:CreateTab("Movement","trending-up")
MT:CreateSection("Walk Speed")
MT:CreateToggle({Name="Enable Walkspeed",CurrentValue=false,Flag="WSE",Callback=function(v) State.WalkspeedEnabled=v; if v then EnableMovement() else ApplySpeed(State.OriginalSpeed) end end})
MT:CreateSlider({Name="Speed",Range={16,150},Increment=1,Suffix=" s/s",CurrentValue=16,Flag="WSV",Callback=function(v) State.WalkspeedValue=v; if State.WalkspeedEnabled then ApplySpeed(v) end end})
MT:CreateSection("Jump Power")
MT:CreateToggle({Name="Enable Jump",CurrentValue=false,Flag="JPE",Callback=function(v) State.JumpEnabled=v; if v then EnableMovement() else ApplyJump(State.OriginalJump) end end})
MT:CreateSlider({Name="Jump Power",Range={50,300},Increment=5,Suffix=" power",CurrentValue=50,Flag="JPV",Callback=function(v) State.JumpValue=v; if State.JumpEnabled then ApplyJump(v) end end})

local FT=Win:CreateTab("FOV","camera")
FT:CreateSection("Field of View")
FT:CreateToggle({Name="Enable FOV",CurrentValue=false,Flag="FOVE",Callback=function(v) State.FOVEnabled=v; if v then State.OriginalFOV=Camera.FieldOfView TweenFOV(State.FOVValue) else TweenFOV(State.OriginalFOV) end end})
FT:CreateSlider({Name="FOV",Range={70,120},Increment=1,Suffix="°",CurrentValue=70,Flag="FOVS",Callback=function(v) State.FOVValue=v; if State.FOVEnabled then TweenFOV(v) end end})
FT:CreateButton({Name="Reset to 70°",Callback=function() State.FOVValue=70; if State.FOVEnabled then TweenFOV(70) end Rayfield:Notify({Title="FOV",Content="Reset to 70°",Duration=2}) end})
...
ST:CreateButton({Name="Destroy & Cleanup",Callback=function()
    DisconnectAll() ClearAll() DisableFullbright() DisableNoFog()
    if State.WalkspeedEnabled then ApplySpeed(State.OriginalSpeed) end
    if State.JumpEnabled then ApplyJump(State.OriginalJump) end
...
Rayfield:Notify({Title="Evade | Visuals",Content="Ready. RightShift = hide GUI.",Duration=4})
